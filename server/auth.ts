...
// A mounted volume preserves local accounts and audit history across restarts.
...
function persistAuth() {
  if (!authDataPath) return;
  fs.mkdirSync(path.dirname(authDataPath), { recursive: true });
  const temp = authDataPath + '.tmp';
  fs.writeFileSync(temp, JSON.stringify({ users: fallbackUsers, logs: activityLogs }), { mode: 0o600 });
  fs.renameSync(temp, authDataPath);
}
