...
export default function VesselDetail({
  ...
  vesselId: number;
  go: (p: Page, vesselId?: number) => void;
  back: () => void;
  onAddReading: (vesselId: number, input: AddReadingInput) => void;
  onAddDelay: (vesselId: number, input: AddDelayInput) => void;
  onComplete: (vesselId: number) => void;
}) {
  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between flex-wrap gap-3 mb-5">
        <div>
          ...
          <div className="text-sm mt-1 text-gray-500">
            {vessel.reference} · {vessel.cargo} · Berth {vessel.berthId}
          </div>
        </div>
      ...
      {isActive && (
        <div className="mt-4">
          <BerthTimeline vessel={vessel} prediction={pred} riskInfo={berthRiskFor(vessel, pred, vessels)} />
        </div>
      )}
