...
export function generateAlerts(params: {
  ...
  const { vessels, predictions, risks, now } = params;
  ...
  for (const v of vessels) {
    ...
    if ((risk?.level === "at-risk" || risk?.level === "delayed") && risk.projectedDelayMin) {
      alerts.push({
        ...
        message: `${v.name} completion forecast has moved ${risk.projectedDelayMin} min later than planned.`,
        vesselId: v.id,
      });
    }
    ...
    if ((v.status === "Unloading" || v.status === "Berthed") && pred) {
      ...
      if (risk2.risk === "conflict") {
        alerts.push({
          ...
          severity: "critical",
          kind: "berth-conflict",
          message: `Berth ${v.berthId} conflict detected between ${v.name} and ${risk2.nextVessel?.name}.`,
          vesselId: v.id,
        });
        ...
        if (diff > 0 && diff <= BERTH_AVAILABLE_SOON_WINDOW_MIN) {
          alerts.push({
            ...
            message: `Berth ${v.berthId} becomes available in approximately ${diff} min.`,
            vesselId: v.id,
          });
        }
      }
