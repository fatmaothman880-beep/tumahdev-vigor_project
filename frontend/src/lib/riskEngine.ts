...
// Provisional display thresholds inherited from M1; not approved site rules.
...
export function computeOperationalRisk(
  ...
  if (vessel.status === "Unloading" || vessel.status === "Berthed" || vessel.status === "Delayed") {
    const current = delays.find((d) => {
      if (d.ongoing) return now.getTime() >= d.start.getTime();
  ...
  if (vessel.status === "Delayed") {
    return {level: "delayed", reason: "Visit marked delayed by an operator", projectedDelayMin: null};
  }
