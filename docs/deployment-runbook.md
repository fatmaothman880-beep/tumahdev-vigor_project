...
Status: Day 6 deployment procedure prepared; execution blocked until the frontend, backend and database branches are integrated.
...
1. Check out the accepted `develop` commit on the demo host.
2. Configure environment values for database, backend URL, frontend URL, authentication and allowed origins.
3. Build and start the integrated services using the team-approved Compose configuration.
4. Apply database migrations and seed only approved synthetic demo records.
...
```powershell
$env:API_BASE_URL = 'https://api.demo.example'
...
| Blocker | Integrated application services and runnable Docker/Python environment are unavailable |
